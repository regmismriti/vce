...
def video_id_from_filename(filename):
    """'fe5d372e43f94f68_988.jpg' -> 'fe5d372e43f94f68'."""
...
def add_paths_and_targets(train_csv="split_1.csv", test_csv="split_0.csv"):
    ...
    # Video-disjoint validation split 
.
    train_df["vid"] = train_df["filename"].map(video_id_from_filename)
    vids_per_class = train_df.groupby("label")["vid"].nunique()
    singleton_classes = set(vids_per_class[vids_per_class <= 2].index)
    if singleton_classes:
        print(f"Classes kept fully in train (<=2 videos): {sorted(singleton_classes)}")
    forced_vids = set(train_df.loc[train_df["label"].isin(singleton_classes), "vid"])
    forced_train = train_df[train_df["vid"].isin(forced_vids)]
    splittable = train_df[~train_df["vid"].isin(forced_vids)]

    skf = StratifiedGroupKFold(n_splits=4, shuffle=True, random_state=CFG["seed"])
    tr_idx, va_idx = next(skf.split(splittable, splittable["label"], groups=splittable["vid"]))
    val_df = splittable.iloc[va_idx].copy()
    train_df = pd.concat([splittable.iloc[tr_idx], forced_train], axis=0).copy()

    overlap = set(train_df["vid"]) & set(val_df["vid"])
    print(f"Video-disjoint split: {len(train_df)} train / {len(val_df)} val frames | "
          f"{train_df['vid'].nunique()} train videos, {val_df['vid'].nunique()} val videos "
          f"(overlap={len(overlap)}) | val covers {val_df['label'].nunique()} classes")
...
class ProgressiveUpsample(nn.Module):
    """
    Progressive 2x upsampling with a learned conv at each step.
...
class MultiScaleCapsuleFeatureFusion(nn.Module):
    """
    Fuses EfficientNetV2 multi-level feature maps with progressive upsampling.

 
      Stage 1 (56x56): reference — no upsample
      Stage 2 (28x28): 28 -> 56          (1 step)
      Stage 3 (14x14): 14 -> 28 -> 56    (2 steps)
      Stage 4  (7x7): 7 -> 14 -> 28 -> 56 (3 steps)
    """
    def __init__(self, in_channels_list, out_channels=256, reference_index=0):
        super().__init__()
        self.reference_index = reference_index
        n = len(in_channels_list)

        # 1x1 projection: each stage -> out_channels
        self.projections = nn.ModuleList([
            ConvBNAct(in_ch, out_channels, kernel_size=1, padding=0)
            for in_ch in in_channels_list
        ])

        # Progressive upsamplers: stage i needs (i - reference_index) 2x steps
        self.upsamplers = nn.ModuleList([
            ProgressiveUpsample(out_channels, n_steps=max(0, i - reference_index))
            for i in range(n)
        ])

        # Fusion: blend all aligned features
        self.fusion = nn.Sequential(
            ConvBNAct(out_channels * n, out_channels, kernel_size=3),
    ...
    def forward(self, features):
        ...
        for feature, projection, upsampler in zip(
                features, self.projections, self.upsamplers):
            feature = projection(feature)
            feature = upsampler(feature)
            # Safety clamp: correct any residual spatial mismatch (odd dims etc.)
...
class DMSEfficientCapsNet(nn.Module):
    ...
    def __init__(
        ...
        self.multi_scale_fusion = MultiScaleCapsuleFeatureFusion(
            ...
            reference_index=0,   # target = Stage1 (56x56), progressive upsample deeper stages
        )
...
def main():
    import argparse
    ap = argparse.ArgumentParser(description="DMSNet — video-disjoint split")
    ap.add_argument("--seed", type=int, default=CFG["seed"])
    ap.add_argument("--weighted_focal", action=argparse.BooleanOptionalAction, default=True,
                    help="class-weighted focal loss (default: True); pass --no_weighted_focal for unweighted")
    args = ap.parse_args()

    CFG["seed"] = args.seed
    seed_everything(CFG["seed"])

    loss_tag = "wf" if args.weighted_focal else "uw"
    result_dir = os.path.join("results", "video_disjoint", f"{loss_tag}_seed{CFG['seed']}")
    os.makedirs(result_dir, exist_ok=True)
    CFG["result_dir"] = result_dir
    CFG["best_model_path"] = os.path.join(result_dir, "best_mcc.pth")
    CFG["h5_model_path"] = os.path.join(result_dir, "best_model.h5")

    print(f"Using device: {DEVICE}")
    print(f"DeformConv2d available: {HAS_DEFORM_CONV}")
    print(f"Run: video-disjoint | loss={'weighted_focal' if args.weighted_focal else 'unweighted'} | seed={CFG['seed']}")
    ...
    if args.weighted_focal:
        criterion = FocalLoss(alpha=class_weights, gamma=CFG["focal_gamma"])
    else:
        criterion = FocalLoss(alpha=None, gamma=CFG["focal_gamma"])
    ...
    val_final = evaluate(model, val_loader, criterion)
    ...
    print(f"[SUMMARY] tag={tag} val_mcc={val_final['mcc']:.4f} val_macroF1={val_final['macro_f1']:.4f} "
          f"test_mcc={test_metrics['mcc']:.4f} test_macroF1={test_metrics['macro_f1']:.4f} "
          f"gap_mcc={val_final['mcc'] - test_metrics['mcc']:.4f}")
    ...
    metrics_out = {
        "seed": CFG["seed"],
        "protocol": "video_disjoint",
        "weighted_focal": args.weighted_focal,
        "test_mcc": round(test_metrics["mcc"], 6),
        "test_macro_recall": round(test_metrics["balanced_acc"], 6),
        "test_macro_f1": round(test_metrics["macro_f1"], 6),
        "test_accuracy": round(test_metrics["acc"], 6),
    }
    with open(os.path.join(CFG["result_dir"], "metrics.json"), "w") as f:
        json.dump(metrics_out, f, indent=2)
    print(f"Metrics saved to {os.path.join(CFG['result_dir'], 'metrics.json')}")
